...
def myRs(tdocimport, modulefolderpath, xscriptcontext):  # 引数は文書のイベント駆動用。複数のリスナーで使用するオブジェクトはここで作成する。
...
class SelectionChangeListener(unohelper.Base, XSelectionChangeListener):
	def __init__(self, xscriptcontext, borders):
		self.xscriptcontext = xscriptcontext
		self.args = borders
...
class ContextMenuInterceptor(unohelper.Base, XContextMenuInterceptor):  # コンテクストメニューのカスタマイズ。
	def __init__(self, xscriptcontext, ctx, smgr, doc):
		self.xscriptcontext = xscriptcontext
